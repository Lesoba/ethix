...
function receiveData(data) {
    ...
    $.each(data, function (key, val) {
        ...
        output += '<li data-role="list-divider">' + val.title.rendered + ' - ' +  val.metadata.PublishDate;
        output += '</li>';
		output += '<li data-role="list-divider vidframe">'
        output += '<div class="vidholder">' + val.content.rendered + '<div>'
		output += '</li>';
        output += '<li data-icon="false">';
		
        output += '<a href="#blogpost" onclick="showPost(' + val.id + ')" data-transition="slide">';
		
        output += val.excerpt.rendered;
        output += '<p class="btn">Click to rate/comment on video.</p></a>';
...
function showPost(id) {
	// AddTrack	
	$.ajax({
            url: 'https://ethix.app/wpapp/queryvid.php?id=' + id + '' ,
            data: {
...
// Detail
    $.getJSON('https://ethix.app/wpapp/wp-json/wp/v2/posts/' + id + '',
		...
		artdet += '<div data-role="list-divider">';
        artdet += '<div class="vidholder">' + data.content.rendered + '<div>';
		artdet += '</div>';
            artdet += '</div>';
            $('#mypost').html(artdet);
// Comment Fields	
